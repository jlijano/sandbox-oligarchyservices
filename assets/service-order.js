(function () {
  const services = [
    ...
    },
    {
      ...
      body: "CRM, ERP, workspace, reporting, and business application coordination that keeps operational systems connected and easier to govern.",
      href: "/business-systems.html",
      cta: "Align systems",
      image: "https://images.unsplash.com/photo-1554224155-6726b3ff858f?auto=format&fit=crop&fm=jpg&q=70&w=1600"
    },
    {
      number: "05",
      title: "ITAD",
    ...
    },
    {
      ...
      href: "/itam.html",
      cta: "Track assets",
      image: "https://images.unsplash.com/photo-1516321318423-f06f85e504b3f?auto=format&fit=crop&fm=jpg&q=70&w=1600"
    }
  ];
  ...
  const carouselStyles = `.services-carousel{max-width:none;margin:0;padding:clamp(78px,9vw,132px) 0;overflow:hidden;background:radial-gradient(circle at 16% 0%,rgba(176,7,20,.26),transparent 28rem),linear-gradient(180deg,#09090a 0%,#171719 100%)}.services-carousel__inner{max-width:1240px;margin:0 auto;padding:0 clamp(18px,4vw,44px)}.services-carousel__header{display:flex;align-items:center;justify-content:space-between;gap:24px;margin-bottom:34px}.services-carousel__controls{display:flex;gap:10px;margin-left:auto}.services-carousel__control{width:46px;height:46px;min-height:46px;border:1px solid rgba(255,255,255,.18);border-radius:999px;background:rgba(255,255,255,.08);color:#fff;font-size:1.5rem;line-height:1;cursor:pointer;transition:background 160ms ease,border-color 160ms ease,transform 160ms ease}.services-carousel__control:hover,.services-carousel__control:focus-visible{border-color:rgba(176,7,20,.78);background:rgba(176,7,20,.5)}.services-carousel__viewport{overflow:hidden;border-radius:8px}.services-carousel__track{display:flex;gap:24px;transform:translate3d(0,0,0);transition:transform 620ms cubic-bezier(.2,.7,0,1);will-change:transform}.services-carousel__card{position:relative;min-height:581px;display:flex;flex:0 0 45%;flex-direction:column;justify-content:flex-end;overflow:hidden;border:1px solid rgba(255,255,255,.13);border-radius:8px;padding:32px;isolation:isolate;text-decoration:none;background:#121214;box-shadow:0 24px 58px rgba(0,0,0,.34);transform:scale(.94);opacity:.62;transition:transform 420ms ease,opacity 420ms ease,border-color 180ms ease,box-shadow 180ms ease}.services-carousel__card.is-active{transform:scale(1);opacity:1}.services-carousel__card::before{content:"";position:absolute;inset:-4%;z-index:-2;background-image:var(--service-card-bg);background-size:cover;background-position:center;filter:saturate(.82) contrast(1.04) brightness(.7);transform:scale(1.06);transition:transform 500ms ease,filter 500ms ease}.services-carousel__card::after{content:"";position:absolute;inset:0;z-index:-1;background:linear-gradient(180deg,rgba(0,0,0,.16),rgba(0,0,0,.56) 44%,rgba(7,7,8,.9))}.services-carousel__card:hover,.services-carousel__card:focus-visible{border-color:rgba(176,7,20,.9);box-shadow:0 26px 68px rgba(0,0,0,.46),0 0 0 1px rgba(176,7,20,.18) inset;outline:none}.services-carousel__card:hover::before,.services-carousel__card:focus-visible::before{filter:saturate(.95) contrast(1.08) brightness(.82);transform:scale(1.1)}.services-carousel__card h3{max-width:12ch;margin:0 0 16px;color:#fff;font-size:clamp(2.1rem,3.2vw,3.15rem);line-height:1.02}.services-carousel__card p{max-width:46ch;margin:0;color:#e4e4e7;font-size:1.08rem;line-height:1.5}.services-carousel__cta{width:max-content;margin-top:28px;border-bottom:2px solid #b00714;color:#fff;font-size:1rem;font-weight:850}.services-carousel__pagination{display:flex;justify-content:center;gap:8px;margin-top:22px}.services-carousel__dot{width:10px;height:10px;border:0;border-radius:999px;background:rgba(255,255,255,.28);cursor:pointer;transition:width 160ms ease,background 160ms ease}.services-carousel__dot[aria-selected="true"]{width:28px;background:#b00714}@media(max-width:940px){.services-carousel__header{justify-content:flex-end}.services-carousel__card{min-height:540px;flex-basis:68%;padding:30px}.services-carousel__card h3{font-size:clamp(1.95rem,6vw,2.75rem)}}@media(max-width:620px){.services-carousel__inner{padding-inline:16px}.services-carousel__track{gap:14px}.services-carousel__card{min-height:527px;flex-basis:100%;padding:27px}.services-carousel__card h3{max-width:14ch;font-size:clamp(1.9rem,10vw,2.55rem)}.services-carousel__card p{font-size:1rem}}`;
  ...
  const installAboutSection = () => {
    ...
    section.innerHTML = `<div class="about-preview__inner"><div class="about-preview__mark" aria-hidden="true"></div><div class="about-preview__copy"><h2 id="about-preview-title">About Us</h2><p>Oligarchy Services helps organizations bring structure to technology operations, from asset lifecycle management and secure handling to user support, managed services, automation, and reporting.</p><p>We work with teams that need practical execution, clearer visibility, and reliable follow-through across the tools and processes that keep the business moving.</p><div class="about-preview__actions"><a class="button primary" href="/about.html" data-track="about_preview_learn_more">Learn More</a></div></div></div>`;
  ...
  const initializeServicesCarousel = (section) => {
    ...
    let index = 0;
    let timer = null;
    const visibleCount = () => window.matchMedia("(max-width: 940px)").matches ? 1 : 2;
    ...
    const goTo = (nextIndex) => { index = clampIndex(nextIndex); render(); };
    const stop = () => { if (timer) window.clearInterval(timer); timer = null; };
    const start = () => { stop(); timer = window.setInterval(() => goTo(index >= maxIndex() ? 0 : index + 1), 3600); };
    prev?.addEventListener("click", () => { goTo(index <= 0 ? maxIndex() : index - 1); start(); });
    next?.addEventListener("click", () => { goTo(index >= maxIndex() ? 0 : index + 1); start(); });
    dots.forEach((dot, dotIndex) => dot.addEventListener("click", () => { goTo(dotIndex); start(); }));
  ...
  const installServicesCarousel = () => {
    ...
    if (existing) { initializeServicesCarousel(existing); return true; }
    ...
    section.innerHTML = `<div class="services-carousel__inner"><div class="services-carousel__header"><p class="eyebrow">Our services</p><div class="services-carousel__controls" aria-label="Service carousel controls"><button class="services-carousel__control" type="button" aria-label="Previous service" data-services-prev>&lsaquo;</button><button class="services-carousel__control" type="button" aria-label="Next service" data-services-next>&rsaquo;</button></div></div><div class="services-carousel__viewport"><div class="services-carousel__track" aria-roledescription="carousel" aria-label="Oligarchy Services service cards">${services.map((card, cardIndex) => `<a class="services-carousel__card" href="${card.href}" role="group" aria-roledescription="slide" aria-label="${cardIndex + 1} of ${services.length}: ${card.title}" style="--service-card-bg: url('${card.image}')"><h3>${card.title}</h3><p>${card.body}</p><span class="services-carousel__cta">${card.cta}</span></a>`).join("")}</div></div><div class="services-carousel__pagination" role="tablist" aria-label="Choose a service card">${services.map((card, cardIndex) => `<button class="services-carousel__dot" type="button" role="tab" aria-label="Go to ${card.title}" aria-selected="${cardIndex === 0 ? "true" : "false"}"></button>`).join("")}</div></div>`;
