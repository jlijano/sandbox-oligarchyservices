...
try {
    ...
    if (table_exists($pdo, 'pages')) {
        ...
        $counts['draft_pages'] = (int) $pdo->query("SELECT COUNT(*) FROM pages WHERE status = 'draft'")->fetchColumn();
    ...
    if (table_exists($pdo, 'navigation_items')) {
        ...
        $counts['visible_nav_items'] = (int) $pdo->query('SELECT COUNT(*) FROM navigation_items WHERE is_visible = 1')->fetchColumn();
        $counts['hidden_nav_items'] = (int) $pdo->query('SELECT COUNT(*) FROM navigation_items WHERE is_visible = 0')->fetchColumn();
    ...
    if (table_exists($pdo, 'activity_log')) {
...
$prevUserParams = array_merge($userFilterParams, ['user_page' => max(1, $userPage - 1)]);
$nextUserParams = array_merge($userFilterParams, ['user_page' => min($totalUserPages, $userPage + 1)]);
$prevUserUrl = '/dashboard.php?' . http_build_query($prevUserParams) . '#users';
$nextUserUrl = '/dashboard.php?' . http_build_query($nextUserParams) . '#users';
    ...
    <div class="dashboard-shell" data-dashboard-shell>
      ...
      <div class="dashboard-main">
        ...
        <main class="dashboard-content">
          ...
          <section class="dashboard-section admin-workspace pages-section" id="pages" data-dashboard-section data-section-label="Pages">
            ...
            <section class="section-summary-grid three-up" aria-label="Page summary">
              <article><span>Total pages</span><strong><?= e((string) $counts['pages']) ?></strong></article>
              <article><span>Published</span><strong><?= e((string) $counts['published_pages']) ?></strong></article>
              <article><span>Drafts</span><strong><?= e((string) $counts['draft_pages']) ?></strong></article>
            </section>
            <div class="panel-grid form-and-table">
          ...
          <section class="dashboard-section admin-workspace navigation-section" id="navigation" data-dashboard-section data-section-label="Navigation">
            ...
            <div class="panel-grid form-and-table">
              ...
              <div class="admin-panel table-panel"><div class="table-heading"><h3>Menu links</h3><span><?= e((string) $counts['nav_items']) ?> link<?= $counts['nav_items'] === 1 ? '' : 's' ?></span></div><?php if (!$navItems): ?><p class="empty-state">No navigation records yet.</p><?php else: ?><div class="table-scroll"><table class="data-table compact-table"><thead><tr><th>Label</th><th>URL</th><th>Sort</th><th>Visible</th><th></th></tr></thead><tbody><?php foreach ($navItems as $row): ?><tr><td><strong><?= e($row['label']) ?></strong></td><td><span class="code-link"><?= e($row['url']) ?></span></td><td class="numeric-cell"><?= e((string) $row['sort_order']) ?></td><td><span class="status-badge <?= (int) $row['is_visible'] === 1 ? 'is-active' : 'is-muted' ?>"><?= (int) $row['is_visible'] === 1 ? 'Visible' : 'Hidden' ?></span></td><td class="row-actions"><button class="table-action" type="button" data-edit-nav data-id="<?= e((string) $row['id']) ?>" data-label="<?= e($row['label']) ?>" data-url="<?= e($row['url']) ?>" data-sort="<?= e((string) $row['sort_order']) ?>" data-visible="<?= e((string) $row['is_visible']) ?>">Edit</button><form method="post" data-confirm="Delete this navigation item?"><input type="hidden" name="csrf_token" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="delete_nav"><input type="hidden" name="return_section" value="navigation"><input type="hidden" name="nav_id" value="<?= e((string) $row['id']) ?>"><button class="table-action danger" type="submit">Delete</button></form></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></div>
            </div>
          </section>
