...
function prospect_sheet_csv_rows(string $csv): array
{
    ...
    if ($handle === false) {
        throw new RuntimeException('Could not open a temporary CSV buffer.');
    }
    fwrite($handle, $csv);
    rewind($handle);
...
function prospect_sheet_payload_from_fields(array $fields, string $defaultStatus, string $sourceLabel): ?array
{
    ...
    if ($first === '' || in_array($first, ['business name', 'company'], true)) {
        return null;
    }

    $warnings = [];
    $additionalNotes = prospect_sheet_text($fields, 17, 5000);
    $dataGaps = prospect_sheet_text($fields, 25, 5000);
    $payload = [
        ...
        'notes' => prospect_sheet_text($fields, 5, 5000),
        'contact' => prospect_sheet_text($fields, 6, 190),
        'email' => prospect_sheet_email($fields, 7, $warnings),
        'phone' => prospect_sheet_text($fields, 8, 80),
        'social_media_links' => prospect_sheet_text($fields, 9, 5000),
        'location' => prospect_sheet_text($fields, 10, 190),
        'reason_relevant' => prospect_sheet_text($fields, 11, 5000),
        'pain_point_trigger' => prospect_sheet_text($fields, 12, 5000),
        'outreach_angle' => prospect_sheet_text($fields, 13, 5000),
        'priority' => prospect_priority(prospect_sheet_text($fields, 14, 40) ?: 'Medium'),
        'last_contact' => prospect_sheet_date($fields, 15),
        'next_step' => prospect_sheet_text($fields, 16, 255),
        'additional_notes' => $additionalNotes,
        'decision_maker_title' => prospect_sheet_text($fields, 18, 190),
        'decision_maker_department' => prospect_sheet_text($fields, 19, 190),
        'decision_maker_profile_url' => prospect_sheet_text($fields, 20, 255),
        'decision_maker_source' => prospect_sheet_text($fields, 21, 5000),
        'contact_confidence' => prospect_contact_confidence(prospect_sheet_text($fields, 22, 40) ?: 'Not Verified'),
        'company_source_url' => prospect_sheet_text($fields, 23, 255),
        'last_verified' => prospect_sheet_date($fields, 24),
...
function prospect_find_existing_id(PDO $pdo, array $payload): ?int
{
    ...
    if (trim((string) ($payload['website'] ?? '')) !== '') {
        $lookups[] = ['SELECT id FROM prospects WHERE website = ? LIMIT 1', [$payload['website']]];
    }
    if (trim((string) ($payload['email'] ?? '')) !== '') {
        $lookups[] = ['SELECT id FROM prospects WHERE email = ? LIMIT 1', [$payload['email']]];
    }
    if (trim((string) ($payload['company'] ?? '')) !== '' && trim((string) ($payload['location'] ?? '')) !== '') {
        $lookups[] = ['SELECT id FROM prospects WHERE LOWER(company) = LOWER(?) AND LOWER(location) = LOWER(?) LIMIT 1', [$payload['company'], $payload['location']]];
    }
    if (trim((string) ($payload['company'] ?? '')) !== '') {
        $lookups[] = ['SELECT id FROM prospects WHERE LOWER(company) = LOWER(?) LIMIT 1', [$payload['company']]];
    }
...
function prospect_sheet_sync_upsert(PDO $pdo, array $payload, int $actorId): string
{
    ...
    prospect_insert($pdo, $payload, $actorId);
    return 'created';
}
...
function prospect_sheet_sync_message(array $summary): string
{
    ...
    if (!empty($summary['errors'])) {
        $message .= ' Errors: ' . implode(' | ', array_map('strval', $summary['errors']));
    }
    return $message;
}
