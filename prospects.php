...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'create_prospect') {
            ...
            $result = prospect_upsert($pdo, $payload, (int) $user['id']);
            $prospectId = (int) $result['id'];
            prospect_log_activity($pdo, (int) $user['id'], 'prospect ' . $result['action'], $prospectId, (string) $payload['company']);
            prospects_flash_success($result['action'] === 'updated' ? 'Matching prospect updated.' : 'Prospect created.');
        ...
        if ($action === 'update_prospect') {
            ...
            $payload = prospect_payload_from_post();
            $duplicateId = prospect_find_existing_id($pdo, $payload, $prospectId);
            if ($duplicateId !== null) throw new RuntimeException('A matching prospect already exists. Open that record instead of saving a duplicate.');
        ...
        if ($action === 'import_prospects') {
            ...
            $updated = 0;
            foreach ($rows as $row) {
                $result = prospect_upsert($pdo, $row, (int) $user['id']);
                if ($result['action'] === 'updated') $updated++;
                else $created++;
            }
            $pdo->commit();
            prospect_log_activity($pdo, (int) $user['id'], 'prospects imported', null, $created . ' created, ' . $updated . ' updated');
            prospects_flash_success($created . ' created, ' . $updated . ' updated.');
            prospects_redirect();
        }
...
$industryCounts = [];
$priorityCounts = [];
$prospectSearch = trim((string) ($_GET['prospect_search'] ?? ''));
$prospectStatus = trim((string) ($_GET['prospect_status'] ?? ''));
$prospectPriority = trim((string) ($_GET['prospect_priority'] ?? ''));
if (!in_array($prospectStatus, array_merge([''], $statuses), true)) $prospectStatus = '';
if (!in_array($prospectPriority, array_merge([''], $priorities), true)) $prospectPriority = '';
$prospectsPerPage = prospects_get_int('per_page', 10);
if (!in_array($prospectsPerPage, [10, 20, 50, 100], true)) $prospectsPerPage = 10;
$prospectPage = max(1, prospects_get_int('prospect_page', 1));
$totalListProspects = 0;
$totalProspectPages = 1;
$prospectStart = 0;
$prospectEnd = 0;

if ($pdo instanceof PDO && $schemaReady) {
    $allStmt = $pdo->prepare('SELECT * FROM prospects ORDER BY updated_at DESC, id DESC');
    $allStmt->execute();
    $allProspects = $allStmt->fetchAll();
    $percentageTotal = 0.0;
    ...
    $params = [];
    if ($prospectSearch !== '') {
        $where[] = '(company LIKE ? OR website LIKE ? OR industry_category LIKE ? OR recommended_services LIKE ? OR notes LIKE ? OR contact LIKE ? OR email LIKE ? OR phone LIKE ? OR social_media_links LIKE ? OR location LIKE ? OR reason_relevant LIKE ? OR pain_point_trigger LIKE ? OR outreach_angle LIKE ? OR next_step LIKE ? OR additional_notes LIKE ?)';
        $searchLike = '%' . $prospectSearch . '%';
        for ($i = 0; $i < 15; $i++) $params[] = $searchLike;
    }
    if ($prospectStatus !== '') {
        $where[] = 'status = ?';
        $params[] = $prospectStatus;
    }
    if ($prospectPriority !== '') {
        $where[] = 'priority = ?';
        $params[] = $prospectPriority;
    }
    $whereSql = $where ? ' WHERE ' . implode(' AND ', $where) : '';
    $countStmt = $pdo->prepare('SELECT COUNT(*) FROM prospects' . $whereSql);
    $countStmt->execute($params);
    $totalListProspects = (int) $countStmt->fetchColumn();
    $totalProspectPages = max(1, (int) ceil($totalListProspects / $prospectsPerPage));
    $prospectPage = min($prospectPage, $totalProspectPages);
    $prospectOffset = ($prospectPage - 1) * $prospectsPerPage;
    $listSql = 'SELECT * FROM prospects' . $whereSql . ' ORDER BY updated_at DESC, id DESC LIMIT ' . $prospectsPerPage . ' OFFSET ' . $prospectOffset;
    $listStmt = $pdo->prepare($listSql);
    $listStmt->execute($params);
    $prospects = $listStmt->fetchAll();
    if ($totalListProspects > 0) {
        $prospectStart = $prospectOffset + 1;
        $prospectEnd = min($totalListProspects, $prospectOffset + count($prospects));
    }
...
if ($prospectSearch !== '') $prospectFilterParams['prospect_search'] = $prospectSearch;
if ($prospectStatus !== '') $prospectFilterParams['prospect_status'] = $prospectStatus;
if ($prospectPriority !== '') $prospectFilterParams['prospect_priority'] = $prospectPriority;
$prospectBaseParams = array_merge($prospectFilterParams, ['per_page' => $prospectsPerPage]);
$prevProspectUrl = '/prospects.php?' . http_build_query(array_merge($prospectBaseParams, ['prospect_page' => max(1, $prospectPage - 1)])) . '#prospect-list';
$nextProspectUrl = '/prospects.php?' . http_build_query(array_merge($prospectBaseParams, ['prospect_page' => min($totalProspectPages, $prospectPage + 1)])) . '#prospect-list';
$csrf = csrf_token();
?>
<!doctype html>
<html lang="en">
      ...
      <div class="dashboard-main">
        ...
        <main class="dashboard-content prospects-workspace">
          ...
          <section class="prospect-action-grid" aria-label="Prospect actions">
            <form class="admin-panel prospect-form" id="prospect-form" method="post"><input type="hidden" name="csrf_token" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="create_prospect"><div class="table-heading"><h3>Add prospect</h3><span>Live database record</span></div><div class="prospect-form-grid"><?php $blank = ['company'=>'','status'=>'New Lead','website'=>'','industry_category'=>'','recommended_services'=>'','conversion_percentage'=>'','notes'=>'','contact'=>'','email'=>'','phone'=>'','social_media_links'=>'','location'=>'','reason_relevant'=>'','pain_point_trigger'=>'','outreach_angle'=>'','priority'=>'Medium','last_contact'=>'','next_step'=>'','additional_notes'=>'','source'=>'Manual Entry','value'=>'0','owner'=>$displayName,'follow_up'=>'','last_activity'=>'Manual prospect entry']; ?><?php include __DIR__ . '/includes/prospect-form-fields.php'; ?></div><button class="button primary" type="submit">Create or update prospect</button></form>
            <form class="admin-panel prospect-form" id="prospect-import" method="post"><input type="hidden" name="csrf_token" value="<?= e($csrf) ?>"><input type="hidden" name="action" value="import_prospects"><div class="table-heading"><h3>Import leads</h3><span>CSV rows</span></div><label>Lead rows<textarea name="import_rows" rows="8" placeholder="Business Name,Status,Website,Industry / Category,Recommended Services,Percentage,Notes,Contact Person,Email,Phone,Social Media Links,Location,Reason Relevant,Pain Point / Buying Trigger,Outreach Angle,Priority,Last Contact,Next Step,Notes,Decision Maker Title,Department / Function,Decision Maker Profile URL,Decision Maker Source,Contact Confidence,Company Source URL,Last Verified,Data Gaps / Validation Notes"></textarea></label><button class="button primary" type="submit">Import leads</button></form>
          </section>
          ...
          <section class="prospect-view is-active" id="prospect-list" data-prospect-view="list"><form class="admin-panel prospect-filters" method="get" action="/prospects.php#prospect-list" data-prospect-filters><label>Search prospects<input type="search" name="prospect_search" value="<?= e($prospectSearch) ?>" data-prospect-search placeholder="Business, contact, industry, service, location"></label><label>Status<select name="prospect_status" data-prospect-status><option value="">All statuses</option><?php foreach ($statuses as $status): ?><option value="<?= e($status) ?>" <?= $prospectStatus === $status ? 'selected' : '' ?>><?= e($status) ?></option><?php endforeach; ?></select></label><label>Priority<select name="prospect_priority" data-prospect-priority><option value="">All priorities</option><?php foreach ($priorities as $priority): ?><option value="<?= e($priority) ?>" <?= $prospectPriority === $priority ? 'selected' : '' ?>><?= e($priority) ?></option><?php endforeach; ?></select></label><label>Show<select name="per_page"><option value="10" <?= $prospectsPerPage === 10 ? 'selected' : '' ?>>10 per page</option><option value="20" <?= $prospectsPerPage === 20 ? 'selected' : '' ?>>20 per page</option><option value="50" <?= $prospectsPerPage === 50 ? 'selected' : '' ?>>50 per page</option><option value="100" <?= $prospectsPerPage === 100 ? 'selected' : '' ?>>100 per page</option></select></label><div class="filter-actions"><button class="button primary" type="submit">Apply</button><a class="secondary-action" href="/prospects.php#prospect-list">Clear</a></div></form><div class="admin-panel table-panel"><div class="table-heading"><h3>Lead pipeline</h3><span data-prospect-result-count><?php if ($totalListProspects > 0): ?>Showing <?= e((string) $prospectStart) ?>-<?= e((string) $prospectEnd) ?> of <?= e((string) $totalListProspects) ?><?php else: ?>0 prospects<?php endif; ?></span></div><?php if (!$prospects): ?><p class="empty-state">No prospects match this view. Add a prospect, import lead rows, or clear filters to see the full CRM workspace.</p><?php else: ?><div class="table-scroll"><table class="data-table prospects-table"><thead><tr><th>Business Name</th><th>Status</th><th>Website</th><th>Industry / Category</th><th>Recommended Services</th><th>Percentage</th><th>Notes</th><th>Contact Person</th><th>Email</th><th>Phone</th><th>Social Media Links</th><th>Location</th><th>Reason Relevant</th><th>Pain Point / Buying Trigger</th><th>Outreach Angle</th><th>Priority</th><th>Last Contact</th><th>Next Step</th><th>Notes</th></tr></thead><tbody><?php foreach ($prospects as $prospect): ?><tr data-prospect-row data-search="<?= e(strtolower(implode(' ', array_map('strval', $prospect)))) ?>" data-status="<?= e((string) $prospect['status']) ?>" data-owner="" data-priority="<?= e((string) $prospect['priority']) ?>"><td><strong><?= e((string) $prospect['company']) ?></strong><small><a class="table-action" href="/prospects.php?open=<?= e((string) $prospect['id']) ?>#prospect-detail">Open</a></small></td><td><span class="prospect-pill status-<?= e(strtolower(str_replace([' ', '/'], '-', (string) $prospect['status']))) ?>"><?= e((string) $prospect['status']) ?></span></td><td><?php if ($prospect['website']): ?><a href="<?= e((string) $prospect['website']) ?>" target="_blank" rel="noopener"><?= e((string) $prospect['website']) ?></a><?php endif; ?></td><td><?= e((string) $prospect['industry_category']) ?></td><td><?= e(prospect_excerpt((string) ($prospect['recommended_services'] ?? ''), 90)) ?></td><td class="numeric-cell"><?= e(prospects_percent($prospect['conversion_percentage'] ?? 0)) ?></td><td><?= e(prospect_excerpt((string) ($prospect['notes'] ?? ''))) ?></td><td><?= e((string) $prospect['contact']) ?></td><td><?php if ($prospect['email']): ?><a href="mailto:<?= e((string) $prospect['email']) ?>"><?= e((string) $prospect['email']) ?></a><?php endif; ?></td><td class="nowrap"><?= e((string) $prospect['phone']) ?></td><td><?= e(prospect_excerpt((string) ($prospect['social_media_links'] ?? ''), 80)) ?></td><td><?= e((string) $prospect['location']) ?></td><td><?= e(prospect_excerpt((string) ($prospect['reason_relevant'] ?? ''))) ?></td><td><?= e(prospect_excerpt((string) ($prospect['pain_point_trigger'] ?? ''))) ?></td><td><?= e(prospect_excerpt((string) ($prospect['outreach_angle'] ?? ''))) ?></td><td><span class="prospect-pill priority-<?= e(strtolower((string) $prospect['priority'])) ?>"><?= e((string) $prospect['priority']) ?></span></td><td class="nowrap"><?= e((string) $prospect['last_contact']) ?></td><td><?= e((string) $prospect['next_step']) ?></td><td><?= e(prospect_excerpt((string) ($prospect['additional_notes'] ?? ''))) ?></td></tr><?php endforeach; ?></tbody></table></div><?php if ($totalProspectPages > 1): ?><nav class="pagination" aria-label="Prospect pages"><a class="secondary-action<?= $prospectPage <= 1 ? ' is-disabled' : '' ?>" href="<?= e($prevProspectUrl) ?>">Previous</a><span>Page <?= e((string) $prospectPage) ?> of <?= e((string) $totalProspectPages) ?></span><a class="secondary-action<?= $prospectPage >= $totalProspectPages ? ' is-disabled' : '' ?>" href="<?= e($nextProspectUrl) ?>">Next</a></nav><?php endif; ?><?php endif; ?></div></section>
          <section class="prospect-view" id="prospect-kanban" data-prospect-view="kanban" hidden><div class="prospect-board" aria-label="Prospect pipeline board"><?php foreach ($kanbanStatuses as $status): $cards = array_values(array_filter($allProspects, fn(array $p): bool => $p['status'] === $status)); ?><section class="kanban-column"><div class="kanban-column-header"><h3><?= e($status) ?></h3><span><?= e((string) count($cards)) ?></span></div><?php foreach ($cards as $card): ?><article class="kanban-card"><div><strong><?= e((string) $card['company']) ?></strong><small><?= e((string) $card['contact']) ?></small></div><div class="kanban-card-meta"><span><?= e(prospects_percent($card['conversion_percentage'] ?? 0)) ?></span><span class="prospect-pill priority-<?= e(strtolower((string) $card['priority'])) ?>"><?= e((string) $card['priority']) ?></span></div><div class="kanban-card-footer"><span><?= e((string) $card['next_step']) ?></span><span class="owner-avatar"><?= e(prospect_initials((string) $card['company'])) ?></span></div><a class="table-action" href="/prospects.php?open=<?= e((string) $card['id']) ?>#prospect-detail">Open</a></article><?php endforeach; ?></section><?php endforeach; ?></div></section>
          <section class="prospect-view" id="prospect-timeline" data-prospect-view="timeline" hidden><div class="timeline-groups"><?php foreach (['Today', 'This week', 'Later'] as $group): ?><section class="admin-panel timeline-group"><div class="table-heading"><h3><?= e($group) ?></h3><span>Last-contact recency</span></div><?php $groupHasRows = false; foreach ($allProspects as $prospect): ?><?php if (prospect_timeline_group($prospect['last_contact'] ?? null) !== $group) continue; $groupHasRows = true; ?><article class="timeline-item"><div class="timeline-date"><?= e((string) ($prospect['last_contact'] ?: 'No date')) ?></div><div><strong><?= e((string) $prospect['company']) ?></strong><small><?= e((string) $prospect['status']) ?> &middot; <?= e((string) $prospect['contact']) ?></small></div><span><?= e(prospects_percent($prospect['conversion_percentage'] ?? 0)) ?></span><span><?= e((string) $prospect['next_step']) ?></span><span class="prospect-pill status-<?= e(strtolower(str_replace([' ', '/'], '-', (string) $prospect['status']))) ?>"><?= e((string) $prospect['status']) ?></span></article><?php endforeach; ?><?php if (!$groupHasRows): ?><p class="empty-state">No prospects in this group.</p><?php endif; ?></section><?php endforeach; ?></div></section>
