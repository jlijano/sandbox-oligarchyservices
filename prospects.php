...
if ($pdo instanceof PDO && $schemaReady) {
    ...
    $params = [];
    if ($prospectSearch !== '') {
        $where[] = '(company LIKE ? OR website LIKE ? OR industry_category LIKE ? OR notes LIKE ? OR contact LIKE ? OR email LIKE ? OR phone LIKE ? OR social_media_links LIKE ? OR location LIKE ? OR reason_relevant LIKE ? OR pain_point_trigger LIKE ? OR outreach_angle LIKE ? OR next_step LIKE ? OR additional_notes LIKE ?)';
        $searchLike = '%' . $prospectSearch . '%';
        for ($i = 0; $i < 14; $i++) $params[] = $searchLike;
    }
    if ($prospectStatus !== '') {
        $where[] = 'status = ?';
